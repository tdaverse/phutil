...
calculating PH with ripser-based engines [@bauer2021ripser; @kaji2020cubical].
...
The {phutil} documentation provides an overview of persistent homology. For now,
know that the result of this procedure is a **persistence diagram**: a multiset
(set with multiplicity) of points in the plane---usually the upper-half plane
and most often the first quadrant. Persistence diagrams encode topological
features of different dimensions, and a single diagram may encode features of
one or all dimensions. By **persistence data**, we mean a data structure that
encodes diagrams of all dimensions, and possibly many diagrams coming from data
sets of a common type.

### Distances between persistence diagrams

Two persistence diagrams $X$ and $Y$ are canonically compared using the
**Wasserstein distance**, parameterized by a $q$-Minkowski distance on the plane
and a $p$-norm on the distances between optimally matched pairs of points:

$$ W_p^q(X,Y) = \inf_{\varphi : X \to Y}{\left( \sum_{x \in X}{{\lVert
x-\varphi(x) \rVert_q}^p} \right)^{1/p}} $$

See the vignette on distances for more detail, or @cohen2010lipschitz and
@bubenik2023exact for detailed treatments and stability results on these
families of metrics.

The interested reader is referred to the related
[vignette](https://tdaverse.github.io/phutil/articles/validation-benchmark.html)
on the website of the package, which also benchmarks and compares the
implementations in {phutil} and {TDA}.
...
classes that have been implemented to handle a single persistence diagram as
well as a set of persistence diagrams.
...
#| fig-align: center
library(ggplot2)
ggplot(as.data.frame(point_data), aes(x = x, y = y)) +
  geom_point() +
  coord_equal() +
  theme_minimal() +
  labs(x = "x", y = "y")
