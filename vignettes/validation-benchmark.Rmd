...
# TODO: Remove explicit tolerance specification once default is shrunk (#32).
...
Note also that `TDA::wasserstein()` does not take the $1/p$th power after computing the sum of $p$th powers; we do this manually to get comparable results:
...
For a straightforward benchmark test, we compute PDs from point clouds sampled with noise from two one-dimensional manifolds embedded in $\mathbb{R}^3$: the circle as a trefoil knot and the segment as a two-armed archimedian spiral.
...
**TODO:** Compute these PDs and save the results rather than generate them fresh. I think this can be done specifically for the vignette, i.e. without making the results an installed data set. I tried caching results[^cache] but could not get it to work.

[^cache]: <https://yihui.org/knitr/options/#cache>
...
Both implementations are used to compute distances between successive pairs of diagrams.
The computations are annotated by homological degree and Wasserstein power so that these results can be compared separately.
...
for (dimension in seq(0, 1)) {
  ...
  bm_1 <- do.call(rbind, lapply(seq_along(PDs1), function(i) {
    bench::mark(
      TDA = wasserstein(PDs1_[[i]], PDs2_[[i]],
                        dimension = dimension, p = 1),
      phutil = wasserstein_distance(PDs1[[i]], PDs2[[i]],
                                    dimension = dimension, p = 1, tol = 1e-8),
      iterations = 1
    )
  }))
  bm_2 <- do.call(rbind, lapply(seq_along(PDs1), function(i) {
    bench::mark(
      TDA = sqrt(wasserstein(PDs1_[[i]], PDs2_[[i]],
                             dimension = dimension, p = 2)),
      phutil = wasserstein_distance(PDs1[[i]], PDs2[[i]],
                                    dimension = dimension, p = 2, tol = 1e-8),
      iterations = 1
    )
  }))
  bm_inf <- do.call(rbind, lapply(seq_along(PDs1), function(i) {
    bench::mark(
      TDA = bottleneck(PDs1_[[i]], PDs2_[[i]],
                       dimension = dimension),
      phutil = bottleneck_distance(PDs1[[i]], PDs2[[i]],
                                   dimension = dimension, tol = 1e-8),
      iterations = 1
    )
  }))
  # annotate
...
Figure \ref{fig:benchmark} compares the distributions of runtimes by homological degree (column) and Wasserstein power (row).
We note that Dionysus via {TDA} clearly outperforms Hera via {phutil} on degree-1 and -2 PDs, which in these cases have many fewer features.
However, the tables are turned in degree 0, in which the PDs have many more features.
...
ggplot2::autoplot(bm_all, type = "violin") +
