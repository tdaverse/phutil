...
The _Kantorovich_ or _Wasserstein metric_[^wasserstein] derives from the problem of optimal transport: What is the minimum cost of relocating one distribution to another?
...
The cost $D(x,\varphi(x))$ of relocating a point $x$ to its matched point $\varphi(x)$ is typically taken to be a Minkowski distance $D_q(x,\varphi(x)) = \lVert x - \varphi(x) \rVert_q$, defined by the $L^q$ norm on $\mathbb{R}^2$.
(While simple, this geometric treatment elides that the points in the plane encode the collection of interval modules into which the persistence module decomposes.
Other metrics have been proposed for this space, but we restrict to this family here.)

The total cost of the relocation is canonically taken to be the Minkowski distance $\left( \sum_{x \in X}{D(x,\varphi(x))^p} \right)^{1/p}$ of the vector of matched-point distances.
The Wasserstein distance is defined to be the infimum of this value over all possible matches.
This yields the formulae
$$W_p^q(X,Y) = \inf_{\varphi : X \to Y}{\left( \sum_{x \in X}{{\lVert x-\varphi(x) \rVert_q}^p} \right)^{1/p}}$$
for $p < \infty$ and
$$W_\infty^q(X,Y) = \inf_{\varphi : X \to Y}{\max_{x \in X}{\lVert x-\varphi(x) \rVert_q}}$$
for $p = \infty$.

See [Cohen-Steiner &al (2010)](https://doi.org/10.1007/s10208-010-9060-6) and [Bubenik &al (2023)](https://doi.org/10.1007/s41468-022-00103-8) for detailed treatments and stability results on these families of metrics.
...
First, with $q=1$, we get $\lVert a \rVert_q = 1+1=2$ and $\lVert b \rVert_q = 0+1=1$. So the $1,p$-Wasserstein distance will be the $p$-Minkowski norm of the vector $(2,1)$, given by $W_p^1(X,Y) = (2^p + 1^p)^{1/p}$. This nets us the values $W_1^1(X,Y) = 3$ and $W_2^1(X,Y) = \sqrt{5}$. And then $W_\infty^1(X,Y) = \max(2,1) = 2$.
...
Let us now validate the bottom row---using the $L^\infty$ distance on the half-plane, giving the popular _bottleneck distance_---using both Hela, as exposed through {phutil}, and Dionysus, as exposed through {TDA}:
...
To validate the distances in this case, we create an empty diagram $E$ and use the same code to compare it to $X$. The point $(3,5)$ of $X$ will be matched to the diagonal $(4,4)$, which yields the same $\infty$-distance $1$ so the $L^\infty$ Wasserstein distances will be the same as before.
...
**REVIEW:** Is it helpful to benchmark on the small data? We're probably picking up differences in data conversion rather than in the algorithms.
...
bm_1$p <- 1; bm_2$p <- 2; bm_inf$p <- Inf
ggplot2::autoplot(rbind(bm_1, bm_2, bm_inf), type = "violin") +
...
For a larger-scale test, we compute PDs from point clouds sampled with noise from two one-dimensional manifolds embedded in $\mathbb{R}^3$: the circle as a trefoil knot and the segment as a two-armed archimedian spiral.
To prevent the results from being sensitive to an accident of a single sample, we generate lists of 24 samples and benchmark only one iteration of each function on each.

```{r compute large PDs current, include=FALSE}
set.seed(28415)
S1 <- tdaunif::sample_trefoil(n = 120, sd = .05)
PD1 <- as_persistence(ripsDiag(S1, maxdimension = 2, maxscale = 6))
S2 <- tdaunif::sample_arch_spiral(n = 120, arms = 2, sd = .05)
PD2 <- as_persistence(ripsDiag(S2, maxdimension = 2, maxscale = 6))
```
...
PDs1 <- lapply(seq(n), function(i) {
  S1 <- tdaunif::sample_trefoil(n = 120, sd = .05)
  as_persistence(ripsDiag(S1, maxdimension = 2, maxscale = 6))
})
PDs2 <- lapply(seq(n), function(i) {
  S2 <- cbind(tdaunif::sample_arch_spiral(n = 120, arms = 2), 0)
  S2 <- tdaunif::add_noise(S2, sd = .05)
  as_persistence(ripsDiag(S2, maxdimension = 2, maxscale = 6))
})
```



```{r benchmark phutil and TDA on large PDs original, include=FALSE}
...
# degree-0
bm_0_1 <- do.call(rbind, lapply(seq_along(PDs1), function(i) {
  bench::mark(
    TDA = wasserstein(PDs1_[[i]], PDs2_[[i]], dimension = 0, p = 1),
    phutil = wasserstein_distance(PDs1[[i]], PDs2[[i]], dimension = 0, p = 1, tol = 1e-8),
    iterations = 1
  )
}))
bm_0_2 <- do.call(rbind, lapply(seq_along(PDs1), function(i) {
  bench::mark(
    TDA = sqrt(wasserstein(PDs1_[[i]], PDs2_[[i]], dimension = 0, p = 2)),
    phutil = wasserstein_distance(PDs1[[i]], PDs2[[i]], dimension = 0, p = 2, tol = 1e-8),
    iterations = 1
  )
}))
bm_0_inf <- do.call(rbind, lapply(seq_along(PDs1), function(i) {
  bench::mark(
    TDA = bottleneck(PDs1_[[i]], PDs2_[[i]], dimension = 0),
    phutil = bottleneck_distance(PDs1[[i]], PDs2[[i]], dimension = 0, tol = 1e-8),
    iterations = 1
  )
}))
bm_0_1$degree <- bm_0_2$degree <- bm_0_inf$degree <- 0
bm_0_1$p <- 1; bm_0_2$p <- 2; bm_0_inf$p <- Inf
# degree-1
bm_1_1 <- do.call(rbind, lapply(seq_along(PDs1), function(i) {
  bench::mark(
    TDA = wasserstein(PDs1_[[i]], PDs2_[[i]], dimension = 1, p = 1),
    phutil = wasserstein_distance(PDs1[[i]], PDs2[[i]], dimension = 1, p = 1, tol = 1e-8),
    iterations = 1
  )
}))
bm_1_2 <- do.call(rbind, lapply(seq_along(PDs1), function(i) {
  bench::mark(
    TDA = sqrt(wasserstein(PDs1_[[i]], PDs2_[[i]], dimension = 1, p = 2)),
    phutil = wasserstein_distance(PDs1[[i]], PDs2[[i]], dimension = 1, p = 2, tol = 1e-8),
    iterations = 1
  )
}))
bm_1_inf <- do.call(rbind, lapply(seq_along(PDs1), function(i) {
  bench::mark(
    TDA = bottleneck(PDs1_[[i]], PDs2_[[i]], dimension = 1),
    phutil = bottleneck_distance(PDs1[[i]], PDs2[[i]], dimension = 1, tol = 1e-8),
    iterations = 1
  )
}))
bm_1_1$degree <- bm_1_2$degree <- bm_1_inf$degree <- 1
bm_1_1$p <- 1; bm_1_2$p <- 2; bm_1_inf$p <- Inf
# # degree-2
# bm_2_1 <- do.call(rbind, lapply(seq_along(PDs1), function(i) {
#   bench::mark(
#     TDA = wasserstein(PDs1_[[i]], PDs2_[[i]], dimension = 2, p = 1),
#     phutil = wasserstein_distance(PDs1[[i]], PDs2[[i]], dimension = 2, p = 1, tol = 1e-8),
#     iterations = 1
#   )
# }))
# bm_2_2 <- do.call(rbind, lapply(seq_along(PDs1), function(i) {
#   bench::mark(
#     TDA = sqrt(wasserstein(PDs1_[[i]], PDs2_[[i]], dimension = 2, p = 2)),
#     phutil = wasserstein_distance(PDs1[[i]], PDs2[[i]], dimension = 2, p = 2, tol = 1e-8),
#     iterations = 1
#   )
# }))
# bm_2_inf <- do.call(rbind, lapply(seq_along(PDs1), function(i) {
#   bench::mark(
#     TDA = bottleneck(PDs1_[[i]], PDs2_[[i]], dimension = 2),
#     phutil = bottleneck_distance(PDs1[[i]], PDs2[[i]], dimension = 2, tol = 1e-8),
#     iterations = 1
#   )
# }))
# bm_2_1$degree <- bm_2_2$degree <- bm_2_inf$degree <- 2
# bm_2_1$p <- 1; bm_2_2$p <- 2; bm_2_inf$p <- Inf
# collate
bm_large <- rbind(
  bm_0_1, bm_0_2, bm_0_inf,
  bm_1_1, bm_1_2, bm_1_inf#,
  # bm_2_1, bm_2_2, bm_2_inf
)
```

```{r visualize large PDs benchmark}
#| fig.width=6, fig.height=3, fig.align='center',
#| fig.cap=
#| "\\label{fig:benchmark-large}
#|  Benchmark comparison of Dionysus via {TDA} and Hera via {phutil}
#|  on large persistence diagrams:
#|  Violin plots of runtime distributions on a common scale."
ggplot2::autoplot(bm_large, type = "violin") +
  ggplot2::labs(x = NULL, y = NULL)
```
