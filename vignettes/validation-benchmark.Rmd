...
vignette: >
  ...
  %\VignetteDepends{bench,ggplot2,TDA}
---
...
autoplot.bench_mark <- function(
  ...
  rlang::check_installed("ggplot2", "for `autoplot()`.")
  type <- match.arg(type)
  if (type == "beeswarm") {
  ...
  # Instead of unnest, expand the nested columns
  res <- data.frame(
    expression = rep(object$expression, sapply(object$time, length)),
    time = unlist(object$time),
    gc = unlist(object$gc)
  )
  
  p <- ggplot2::ggplot(res)
  switch(type,
         beeswarm = p <- p + ggplot2::aes(.data$time, .data$expression, color = .data$gc) + 
           ggbeeswarm::geom_quasirandom(..., orientation = "y"),
         jitter = p <- p + ggplot2::aes(.data$time, .data$expression, color = .data$gc) + 
           ggplot2::geom_jitter(...),
         ridge = p <- p + ggplot2::aes(.data$time, .data$expression) + 
           ggridges::geom_density_ridges(...),
         boxplot = p <- p + ggplot2::aes(.data$time, .data$expression) + 
           ggplot2::geom_boxplot(...),
         violin = p <- p + ggplot2::aes(.data$time, .data$expression) + 
           ggplot2::geom_violin(...))
  
  parameters <- setdiff(colnames(object), 
                       c("expression", summary_cols, data_cols, 
  ...
  if (length(parameters) == 2) {
    return(p + ggplot2::facet_grid(paste0(parameters[[1]], "~", parameters[[2]]), 
...
The _Kantorovich_ or _Wasserstein metric_[^wasserstein] derives from the problem
...
See [Cohen-Steiner &al (2010)](https://doi.org/10.1007/s10208-010-9060-6) and
[Bubenik &al (2023)](https://doi.org/10.1007/s41468-022-00103-8) for detailed
treatments and stability results on these families of metrics.
...
Note that the vector between the off-diagonal points $(1,3)$ of $X$ and $(3,4)$
of $Y$ is $(2,1)$, while the vector from $(1,3)$ to its nearest diagonal point
$(2,2)$ is $(1,-1)$. That one coordinate is the same size while the other is
smaller implies that an optimal matching will always match $(1,3)$ with the
diagonal, so long as $p \geq 1$. A similar argument necessitates that $(3,4)$ of
...
\max(2,1) = 2$. The reader is invited to complete the rest of Table
\ref{tab:small}.
...
$L^\infty$  &        $1$        &         $1$         &
           $2$ & $\sqrt{2}$        &         $1$         \\
\end{tabular}
\caption{\label{tab:small}Distances between optimally paired features and
Wasserstein distances between $X$ and $Y$ for several choices of half-plane and
...
# distance from empty using phutil/Hera
wasserstein_distance(as_persistence(Z), as_persistence(W), p = 1, dimension = 2)
wasserstein_distance(as_persistence(Z), as_persistence(W), p = 2, dimension = 2)
bottleneck_distance(as_persistence(Z), as_persistence(W), dimension = 2)
# distance from empty using TDA/Dionysus
...
Figure \ref{fig:benchmark} compares the distributions of runtimes by homological
degree (column) and Wasserstein power (row).
We note that Dionysus via {TDA} clearly outperforms Hera via {phutil} on
degree-1 and -2 PDs, which in these cases have many fewer features.
However, the tables are turned in degree 0, in which the PDs have many more
features. While by no means exhaustive and not necessarily representative, these
results suggest that Hera via {phutil} scales more efficiently than Dionysus via
{TDA} and should therefore be preferred for projects involving more feature-rich
data sets.

```{r visualize large PDs benchmark}
#| fig.width=6, fig.height=3, fig.align='center', fig.retina=2,
#| fig.cap=
#| "\\label{fig:benchmark}
