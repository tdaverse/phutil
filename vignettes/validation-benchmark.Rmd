...
points(pd1, pch = 1)
points(pd2, pch = 5)
segments(pd1[, 1], pd1[, 2], c(2, pd2[, 1]), c(2, pd2[, 2]), lty = 2)
par(mar = c(5, 4, 4, 2) + .1)
```
...
An important edge case for these functions is when one persistence diagram is trivial, i.e. contains only the diagonal so is "empty" of off-diagonal points.
...
To validate the distances in this case, we create an empty matrix of persistent pairs and run the same code:

```{r validate small PD vs empty}
# TODO: Change default to `tol = 1e-8` (floating-point precision).
# empty PD
...
# distance from empty using phutil/Hera
wasserstein_distance(as_persistence(pd3), as_persistence(pd4), p = 1, dimension = 2, tol = 1e-8)
wasserstein_distance(as_persistence(pd3), as_persistence(pd4), p = 2, dimension = 2, tol = 1e-8)
bottleneck_distance(as_persistence(pd3), as_persistence(pd4), dimension = 2, tol = 1e-8)
# distance from empty using TDA/Dionysus
wasserstein(pd3, pd4, p = 1, dimension = 2)
sqrt(wasserstein(pd3, pd4, p = 2, dimension = 2))
bottleneck(pd3, pd4, dimension = 2)
```
...
bm1 <- bench::mark(
  TDA = wasserstein(dpd1, dpd2, dimension = 0, p = 1),
  phutil = wasserstein_distance(pd1, pd2, p = 1, tol = 1e-8)
)
bm2 <- bench::mark(
  TDA = sqrt(wasserstein(dpd1, dpd2, dimension = 0, p = 2)),
  phutil = wasserstein_distance(pd1, pd2, p = 2, tol = 1e-8)
)
bminf <- bench::mark(
  TDA = bottleneck(dpd1, dpd2, dimension = 0),
  phutil = bottleneck_distance(pd1, pd2, tol = 1e-8)
)
