...
# degree-0
bm_0_1 <- bench::mark(
  TDA = wasserstein(PD1_, PD2_, dimension = 0, p = 1),
  phutil = wasserstein_distance(PD1, PD2, dimension = 0, p = 1, tol = 1e-8)
)
bm_0_2 <- bench::mark(
  TDA = sqrt(wasserstein(PD1_, PD2_, dimension = 0, p = 2)),
  phutil = wasserstein_distance(PD1, PD2, dimension = 0, p = 2, tol = 1e-8)
)
bm_0_inf <- bench::mark(
  TDA = bottleneck(PD1_, PD2_, dimension = 0),
  phutil = bottleneck_distance(PD1, PD2, dimension = 0, tol = 1e-8)
)
...
```{r benchmark phutil and TDA on large PDs experimental, include=FALSE}
#| fig.width=6, fig.height=3, fig.align='center',
#| fig.cap=
#| "\\label{fig:benchmark-large}
#|  Benchmark comparison of Dionysus via {TDA} and Hera via {phutil}
...
# TODO: Write `as.matrix()` method for "persistence" class.
# TODO: Write `as_diagram()` function for "persistence" objects.
# FIXME: Ensure that empty PDs (degree-2) are handled correctly.
PD1_ <- as.matrix(as.data.frame(PD1)); PD2_ <- as.matrix(as.data.frame(PD2))
# apply over degrees
# TODO: Switch to `seq(0, 2)` once empty diagrams are handled correctly.
bm_large <- lapply(seq(0, 1), function(deg) {
  bm_1 <- bench::mark(
    TDA = wasserstein(PD1_, PD2_, dimension = deg, p = 1),
    phutil = wasserstein_distance(PD1, PD2, dimension = deg, p = 1, tol = 1e-8)
  )
  bm_2 <- bench::mark(
    TDA = sqrt(wasserstein(PD1_, PD2_, dimension = deg, p = 2)),
    phutil = wasserstein_distance(PD1, PD2, dimension = deg, p = 2, tol = 1e-8)
  )
  bm_inf <- bench::mark(
    TDA = bottleneck(PD1_, PD2_, dimension = deg),
    phutil = bottleneck_distance(PD1, PD2, dimension = deg, tol = 1e-8)
  )
  bm_1$degree <- bm_2$degree <- bm_inf$degree <- deg
  bm_1$p <- 1; bm_2$p <- 2; bm_inf$p <- Inf
  rbind(bm_1, bm_2, bm_inf)
})
bm_large <- do.call(rbind, bm_large)
# plot
ggplot2::autoplot(bm_large, type = "violin") +
  ggplot2::labs(x = NULL, y = NULL)
```
