...
vignette: >
  ...
  %\VignetteDepends{TDA,bench,ggplot2,scales}
---
...
Below, remember that the functions with the `*_distance()` suffix come from {phutil}, while those without come from {TDA}.
...
library(ggplot2)
bm_all %>%
  transform(implementation = as.character(expression), time = unlist(time)) %>%
  subset(select = c(implementation, degree, power, time)) %>%
  ggplot(aes(x = time, y = implementation)) +
  facet_grid(
